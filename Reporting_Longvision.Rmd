...
{
#get Pattern
data.Performer.pattern.AbsolutPrices<- Pattern.CreateData(data.Performer.clean,data.History,Symbol,Timeframe,Norm=FALSE)

#Write File
write.csv(data.Performer.pattern.AbsolutPrices,file=FilePath)
}else
{
  data.Performer.pattern.AbsolutPrices<- read.csv(FilePath,stringsAsFactors=TRUE)  
}
head(data.Performer.pattern.AbsolutPrices,5)
...
{
  #get Pattern
  data.Performer.pattern.ScaledPrice<- Pattern.CreateData(data.Performer.clean,data.History,Symbol,Timeframe,Norm=TRUE) 
  
  #Write File
  write.csv(data.Performer.pattern.ScaledPrice,file=FilePath)
}else
{
  data.Performer.pattern.ScaledPrice<- read.csv(FilePath,stringsAsFactors=TRUE)
}
head(data.Performer.pattern.ScaledPrice,5)
...
if(!file.exists(FilePath))
{
  ...
  data.Pattern.Entries<-Pattern.Entries(ColumnsCleanData, data.Performer.pattern.ScaledPrice)
  
  #Write File
  write.csv(data.Pattern.Entries,file=FilePath)
}else
{
  data.Pattern.Entries<- read.csv(FilePath,stringsAsFactors=FALSE)
}
...
Falls Es mehrere Strategien sind muss erstma das Data Geclustered werden:

```{r Clustering}


dat<-data.Pattern.Entries[,-(1:3)]
f<-complete.cases(dat)
dat<-dat[f,]
...
wss <- (nrow(mydata)-1)*sum(apply(mydata,2,var))
  for (i in seq(from=2, to=itterations, by=1)) wss[i] <- sum(kmeans(mydata,
                                       centers=i,iter.max = 100)$withinss)
plot(1:itterations, wss, type="b", xlab="Number of Clusters",
     ylab="Within groups sum of squares",
     main="Assessing the Optimal Number of Clusters with the Elbow Method",
     pch=20, cex=2)


km10 = kmeans(dat, 10, nstart=100,iter.max = 100)
View()
